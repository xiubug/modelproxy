...
ModelProxy.prototype = {
    ...
    // 针对动态调用方法，做个封装
    invoke:function(map,key){
        var intfs = (map[key] && map[key].interfaces) || [];
        for (var i = 0; i < intfs.length; i++) {
            var intf = intfs[i];
            if(!intf.name){
                continue;
            }else{
                var proxy = ProxyFactory.create(this.profile[intf.name]);
                var params = intf.params || [];
                var expire = intf.expire || 0;
                if ( !this._queue ) {
                   this._queue = [];
                }
                this._queue.push( {
    ...
    _sendRequestsParallel: function( queue, callback, errCallback ) {
        ...
        for ( var i = 0; i < queue.length; i++ ) {
            ( function( reqObj, k, cookie ) {
                ...
                reqObj.proxy.request( reqObj, function( data, setcookie ) {
